...
from sys import stdout, stderr, exit, setrecursionlimit, maxsize
...
def copy_segment(startPos, prev=None, stopAt=None, numberOfGenes=0):
    ...
    if stopAt == None and not isinstance(startPos, Telomere):
        stopAt = startPos

    if numberOfGenes == 0:
        numberOfGenes = maxsize
    ...
    copyPrev = copyChr
    if startPos.orient(prev) == DIRECTION_CRICK_STRAND:
...
def copy_genome(genome):
    ...
    for chromosome in genome:
        ...
        if not isinstance(chrStart, Telomere):
            stickyEnd(chrStart)
            stickyStart(chrEnd)
        res.append(chrStart)
    return res
...
def checkIntegrity(genome):
    ...
    for i, chrx in enumerate(genome):
        if isinstance(chrx, Telomere) and chrx.tail != None:
            LOG.error('telomeres are not supposed to have ' + \
                f'tail connections, but {cur} has')
        ...
        c = 0
        while cur != chrx.tail:
            if isinstance(cur, Telomere) and cur.tail != None:
                LOG.error('telomeres are not supposed to have ' + \
                    f'tail connections, but {cur} has')
                import pdb; pdb.set_trace()
            ...
            if prev != None:
                if cur_orient == DIRECTION_CRICK_STRAND:
                    if prev_orient == DIRECTION_CRICK_STRAND and \
                        (prev.head != cur or cur == None or cur.tail != prev):
                    ...
                    if prev_orient == DIRECTION_WATSON_STRAND and \
                        (prev.tail != cur or cur == None or cur.tail != prev):
                        LOG.error(('integrity test of chromosome %s failed ' + \
                    ...
                    if prev_orient == DIRECTION_CRICK_STRAND and \
                        (prev.head != cur or cur == None or cur.head != prev):
                        LOG.error(('integrity test of chromosome %s failed ' + \
                    ...
                    if prev_orient == DIRECTION_WATSON_STRAND and \
                        (prev.tail != cur or cur == None or cur.head != prev):
                        LOG.error(('integrity test of chromosome %s failed ' + \
...
def seq_cut(chromosome, pos):

    seq = chromosome.map_reduce(lambda x: x, lambda x, y: [(x.orient(y[0][1]),
        x)] + y, init=[(DIRECTION_CRICK_STRAND, chromosome.tail and chromosome
            or None)], incoming=chromosome.tail)
    stickyEnd1 = partial((seq[pos-1][0] == DIRECTION_CRICK_STRAND or
        seq[pos-1][0] == DIRECTION_BOTH_STRAND) and joinHead or joinTail,
        seq[pos-1][1])
    stickyEnd2 = partial((seq[pos][0] == DIRECTION_CRICK_STRAND or seq[pos][0]
        == DIRECTION_BOTH_STRAND) and joinTail or
            joinHead, seq[pos][1])

    return stickyEnd1, stickyEnd2, seq[pos-1][1], seq[pos][1]
...
def sampleSites(genome, chr_lengths, number_of_sites=1, min_dist=0):
    ...
    # than min_dist
    feasible_chrs = [x for x in range(len(genome)) if chr_lengths[x]
...
#                        and chr_lengths[site[0]] == 2):
            ...
            while np.random.random() < ir:
                ...
                LOG.info('++ insertion of sequence (%s) at site Chr%s:%s' %( \
                        ' '.join(map(str, range(gfCount + 1, gfCount + 1 + \
            ...
            info_msg = '++ double cut (Chr%s[%s|%s], Chr%s[%s|%s])' %(chr1+1, p1,
                    p1+1, chr2+1, p2, p2+1)
            try: 
                seq_stickyEnd1, seq_stickyEnd2, seq_end1, seq_end2 = seq_cut(genome[chr1], p1)
                stickyEnd1, stickyEnd2, end1, end2 = cut(genome[chr1], p1)
                seq_stickyEnd3, seq_stickyEnd4, seq_end3, seq_end4 = seq_cut(genome[chr2], p2)
                stickyEnd3, stickyEnd4, end3, end4 = cut(genome[chr2], p2)

#                if any(*map(lambda x: instanceof(x, Telomere), (end1, end2,
#                    end3, end4))):
#                    import pdb; pdb.set_trace() 
                if (end1, end2, end3, end4) != (seq_end1, seq_end2, seq_end3,
                        seq_end4):
                    import pdb; pdb.set_trace() 
                LOG.debug(f'end1={end1}, end2={end2}, end3={end3}, end4={end4}')
            ...
            if chr1 == chr2:
                if join_combination == 1:
                    ...
                    info_msg += (' results in inversion of between %s and %s ' \
                            + 'of chromosome Chr%s') %(end1, end4, chr1+1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('newick_tree', type=open, default=0,
            help='rate of indels, measured relative to DCJ events')
...
#            exit(1)
    ...
    if args.out_file == None:
        ...
        print(dcj_tree, file=treeOutHandle)
        treeOutHandle.close()
